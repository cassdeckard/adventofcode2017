...
class HexPosition {
    ...
    private static var memo = [HexPosition.stringify(0, 0) : HexPosition(x: 0, y: 0)]
    
    static func stringify(_ x: Int, _ y: Int) -> String {
        return "(\(x), \(y))"
    }
    ...
    static var origin : HexPosition {
        return HexPosition.make(0, 0)
    }
    
    func moved(_ direction: Direction) -> HexPosition {
        return HexPosition.make(x + direction.offsets.0,
                                y + direction.offsets.1)
    }
...
extension HexPosition : CustomDebugStringConvertible {
    var debugDescription : String {
        return HexPosition.stringify(x, y)
    }
}

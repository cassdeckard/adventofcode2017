...
func solve(_ input: [Int]) -> UInt {
    var step : UInt = 0
    var target = 0
    var instructions = input
    
    while target >= 0 && target < instructions.count {
        step += 1
        let offset = instructions[target]
        let nextTarget = target + offset
        instructions[target] += (offset >= 3 ? -1 : 1)
        target = nextTarget
        if step % 100000 == 0 {
            print(instructions)
            print(step)
